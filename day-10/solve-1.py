...
def solve(input_file) -> tuple[int, int]:
    ...
    for pos in puzzle:
        if puzzle[pos] == 0:
            answer += get_score(puzzle, pos, 0)

    return answer, process_time_ns() - start_time


def get_score(puzzle: Matrix, current_pos: Vec2, current_level: int, found_ends: set[Vec2] = None) -> int:
    if found_ends is None:
        found_ends = set()

    if current_level == 9:
        found_ends.add(current_pos)
        return len(found_ends)

    next_level = current_level + 1
    for direction in Directions:
        next_pos = current_pos + direction.value
        if puzzle[next_pos] == next_level:
            get_score(puzzle, next_pos, next_level, found_ends)
