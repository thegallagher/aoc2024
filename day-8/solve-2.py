    ...
    for frequency in frequencies:
        ...
        for antenna_a in antennas:
            for antenna_b in antennas:
                ...
                antinode_a = vec2_add(antenna_b, delta)
                while 0 <= antinode_a[0] < width and 0 <= antinode_a[1] < height:
                    antinodes.add(antinode_a)
                    antinode_a = vec2_add(antinode_a, delta)

                antinode_b = vec2_sub(antenna_a, delta)
                while 0 <= antinode_b[0] < width and 0 <= antinode_b[1] < height:
                    antinodes.add(antinode_b)
                    antinode_b = vec2_sub(antinode_b, delta)
